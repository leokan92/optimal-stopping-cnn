...
def Becker_train_model(s_0,K,T,N,r,delta,sigma,d,batch_size,order,type_of_data,
                     PATH,num_neurons,lr_boundaries,lr_init,training_steps,file,path,path_output,asset):
...
def BeckerH_train_model(s_0,K,T,N,r,delta,sigma,d,batch_size,order,type_of_data,
    ...
    for train_step in range(0,training_steps+1):
        ...
        if type_of_data == 'real_val':
            ...
            data_train = 'none'
        input_gen = select_input(s_0,K,T,N,r,delta,sigma,d,batch_size,seed,type_of_data,order,data_train,file,path)
        X,p_,g_tau = gen_func(input_gen)
        ...
        loss = torch_t(loss)
        state_hist = []
        #hist_state = -torch.ones((state.shape[0],state.shape[1],N)).to(device)*1000
        hist_state = torch.zeros((state.shape[0],state.shape[1],N)).to(device)
        for n in range(0, N, 1):
            state_for_append = torch.cat((hist_state[:,:,n+1:],torch.flip(state[:,:,:n+1],dims=[0,2])), dim=2)
            state_hist.append(state_for_append.view(state.shape[0],-1))
        ...
        if type_of_data == 'real_val':
            ...
            if train_step%100 == 0 or train_step <100:
                    print('| Train step: {:5.0f} | Loss: {:3.3f} | Avg price Train: {:3.3f} | Lr: {:1.6f} |'.format(train_step,loss.item(),px_hist[-1],optimizer.param_groups[0]['lr']))
                
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        scheduler.step()
    np.save(path_output+'Becker_train_'+str(N)+'_'+asset,np.asarray(px_hist))
    np.save(path_output+'Becker_val_'+str(N)+'_'+asset,np.asarray(px_hist_val))
...
def Becker_mod_cnn_train_model(s_0,K,T,N,r,delta,sigma,d,batch_size,order,type_of_data,
    ...
    for train_step in range(0,training_steps+1):
        ...
        if type_of_data == 'real_val':
            if train_step> 5:
                if px_hist_val[-1]>max_px and px_hist_val[-1]<np.mean(px_hist_val[-5:-1])*1.70:
                    torch.save(neural_net.state_dict(), path_output+'best_model_cnn.pt')
        else: 
            if train_step> 5:
                if px_mean_batch.item()>max_px and px_mean_batch.item()<np.mean(px_hist[-10:-1])*1.70:
                    torch.save(neural_net.state_dict(), path_output+'best_model_cnn.pt')
        ...
        optimizer.step()
        scheduler.step()
    np.save(path_output+'Becker_cnn_train_'+str(N)+'_'+asset,np.asarray(px_hist))
    np.save(path_output+'Becker_cnn_val_'+str(N)+'_'+asset,np.asarray(px_hist_val))
