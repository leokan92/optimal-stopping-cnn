...
from utils import choose_gen,torch_t,select_input,calculate_avg_payoff,write_file,write_table
...
def Becker_test_model(s_0,K,T,N,r,delta,sigma,d,batch_size,order,type_of_data,
                     PATH,num_neurons,lr_boundaries_NN,lr_init,mc_runs,train_steps,file,path):
    ...
    neural_net.load_state_dict(torch.load(PATH+'/best_model_becker.pt'))
    neural_net.eval()  
    ...
    for mc_step in range(0,mc_runs+1):
        ...
        input_gen = select_input(s_0,K,T,N,r,delta,sigma,d,batch_size,seed,type_of_data,order,file,path)
